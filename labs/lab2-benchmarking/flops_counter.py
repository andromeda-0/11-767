...
def get_electra_train_flops(h_d, l_d, h_g, l_g, batch_size, train_steps,
    ...
    gen = BERTHparams(h_g, l_g, s=s, e=e if tied_embeddings else None).get_train_flops(batch_size,
                                                                                       train_steps)
    return disc + gen
...
model_flops = collections.OrderedDict([
    ...
    ("roberta_base_h256",
     BERTHparams(256, 12, v=50265, s=128, i=1024, heads=4).get_train_flops(8192, 1)),
    ("roberta_base_h512",
     BERTHparams(512, 12, v=50265, s=128, i=2048, heads=8).get_train_flops(8192, 1)),
    ("roberta_base_h1024",
     BERTHparams(1024, 12, v=50265, s=128, e=768, i=4096, heads=16).get_train_flops(8192, 1)),
    ("roberta_base_h1536",
     BERTHparams(1536, 12, v=50265, s=128, e=768, i=6144, heads=24).get_train_flops(8192, 1))
